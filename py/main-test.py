...
configPath = "/home/ivan/Projects/MagneticField/config-circle.yaml"
stream = open(configPath, 'r')
Val = yaml.load(stream)   #  Python-словарь со значениями из config.yaml
"------------------Начальные значения для первой катушки----------------------"
coilRadius1 = Val["coilRadius1"] 
loopCurrent1 = Val["loopCurrent1"]
...
coilRadius2 = Val["coilRadius2"] 
loopCurrent2 = Val["loopCurrent2"]
...
for step in range(1, 50, 1):
    ...
    planeSide += 50    
    "------------------Создаем катушки----------------------------------------"
    c1 = CoilLoops(isLeft1, coilRadius1, loopCurrent1, nLayers1\
                , layerStep1, nLoops1, loopStep1, distance1)
    coil1 = c1.make_coil()
    c2 = CoilLoops(isLeft2, coilRadius2, loopCurrent2, nLayers2\
                , layerStep2, nLoops2, loopStep2, distance2)
    coil2 = c2.make_coil()
